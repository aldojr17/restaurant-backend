...
func (repo *userRepository) GetUserById(id string) *domain.Response {
	...
	if err := repo.db.Preload("Favorites.Menu.Category").Preload("Favorites.Menu.MenuOption").Preload("Favorites.Menu.Reviews").Table("users").Where("id", id).First(&user).Error; err != nil {
